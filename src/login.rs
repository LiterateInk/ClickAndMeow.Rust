...
pub async fn login(username: &str, password: &str) -> Result<Session, Error> {
    ...
    headers.insert("Cookie", format!("PHPSESSID={}", login_session_id).parse().unwrap());
    ...
    let php_session_id = response.headers.get_all("Set-Cookie").into_iter().find(|x| x.to_str().unwrap().starts_with("PHPSESSID=")).unwrap()
                                .to_str().unwrap().split(";").next().unwrap().split("=").skip(1).next().unwrap();
